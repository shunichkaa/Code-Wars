...
class Warrior {
	...
	level() {
		return Math.min(100, Math.floor(this._experience / 100));
	}

	rank() {
		return this._ranks[Math.floor(this.level() / 10)];
	}
	...
	training(event) {
		const [description, expGain, minLevel] = event;
		if (this.level() >= minLevel) {
			this._addExperience(expGain);
	...
	battle(enemyLevel) {
		...
		if (levelDiff >= 5 && this._rankIndex(myRank) < this._rankIndex(enemyRank)) {
			return "You've been defeated";
		}

		let expGain = 0;

		if (levelDiff === 0) expGain = 10;
		else if (levelDiff === -1) expGain = 5;
		else if (levelDiff <= -2) expGain = 0;
		else if (levelDiff > 0) expGain = 20 * levelDiff * levelDiff;

		if (this.level() === 100) expGain = 0;

		this._addExperience(expGain);

		if (levelDiff <= -2) return "Easy fight";
		if (levelDiff === 0 || levelDiff === -1) return "A good fight";
		return "An intense fight";
	}

	_addExperience(amount) {
		if (this._experience >= this._maxExp) return;
		this._experience += amount;
		if (this._experience > this._maxExp) this._experience = this._maxExp;
	}

	_rankIndex(rank) {
		return this._ranks.indexOf(rank);
	}
}