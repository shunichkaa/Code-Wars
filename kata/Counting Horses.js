...
function countHorses(sound) {
	let arr = sound.split('').map(Number);
	const result = [];
	const length = arr.length;

	while (arr.some(x => x > 0)) {
		let found = false;
		for (let leg = 1; leg <= length; leg++) {
			// Проверяем, может ли лошадь с длиной ноги leg быть в этом звуке
			let possible = true;
			for (let pos = leg - 1; pos < length; pos += leg) {
				if (arr[pos] <= 0) {
					possible = false;
					break;
				}
			...
			if (possible) {
				// Учитываем лошадь с длиной ноги leg, вычитаем 1 из соответствующих позиций
				for (let pos = leg - 1; pos < length; pos += leg) {
					arr[pos]--;
				}
				result.push(leg);
				found = true;
				break; // найдена одна лошадь, повторяем поиск дальше
			}
		}
		// Если не нашли ни одной лошади — значит звук не распознаётся корректно
		if (!found) break;
	}
	return result;
}