...
function findReverseNumber(n) {
	...
	n -= 10n; // убираем палиндромы длины 1

	let length = 2n;
	while (true) {
		// Кол-во палиндромов для длины length
	...
	let start = pow10(halfLen - 1n); // первая левая половина палиндрома для этой длины
	let leftNum = start + (n - 1n); // индексируем с 0
