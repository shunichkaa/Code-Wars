...
function PNum(n) {
	if (n <= 0) return false;
	const val = (1 + Math.sqrt(1 + 24 * n)) / 6;
	return isInteger(val) && val > 0;
}

function GPNum(n) {
	if (n < 0) return false; // Generalized pentagonal numbers не могут быть отрицательными (число точек)
	const sqrtTerm = Math.sqrt(1 + 24 * n);
	if (!isInteger(sqrtTerm)) return false;

	// Проверяем оба варианта k:
	const k1 = (1 + sqrtTerm) / 6;
	const k2 = (1 - sqrtTerm) / 6;

	return isInteger(k1) || isInteger(k2);
}
