...
function findTheKey(message, code) {
	...
	for (let i = 0; i < message.length; i++) {
		const letterValue = message.charCodeAt(i) - aCharCode + 1;
		keyDigits.push(code[i] - letterValue);
	}

	for (let len = 1; len <= keyDigits.length; len++) {
		const pattern = keyDigits.slice(0, len);
		let match = true;

		for (let i = 0; i < keyDigits.length; i++) {
			if (keyDigits[i] !== pattern[i % len]) {
				match = false;
				break;
			}
		}
