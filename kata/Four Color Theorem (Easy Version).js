...
// 	[1] Dailey, D. P. (1980), "Uniqueness of colorability and colorability of planar 4-regular graphs are NP-complete", Discrete Mathematics, 30 (3): 289–293
//
...
function color(testmap) {
	...
	// Индекс для каждой территории
	const idx = new Map();
	nodes.forEach((v, i) => idx.set(v, i));

	// Строим граф смежности (adjacency list)
	...
	// Backtracking: пытаемся раскрасить граф с k цветами
	function canColor(k) {
		...
		function dfs(v = 0) {
			if (v === n) return true;
			for (let c = 1; c <= k; c++) {
				// Проверяем, можно ли покрасить v в цвет c
				let ok = true;
	...
	// Перебираем от 1 до 4 цветов (по теореме)
	for (let k = 1; k <= 4; k++) {
		if (canColor(k)) return k;
	}

	return 4; // по теореме максимум 4
}
