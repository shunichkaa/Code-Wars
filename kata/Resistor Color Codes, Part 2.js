...
function encodeResistorColors(ohms) {
	const colors = ['black', 'brown', 'red', 'orange', 'yellow', 'green', 'blue', 'violet', 'gray', 'white'];
	let valueStr = ohms.split(' ')[0];
	let multiplier;

	if (valueStr.endsWith('k')) {
		valueStr = parseFloat(valueStr) * 1000;
	} else if (valueStr.endsWith('M')) {
		valueStr = parseFloat(valueStr) * 1000000;
	} else {
		valueStr = parseFloat(valueStr);
	}

	// Приводим к целому числу (значения будут целыми, как в условии)
	const value = Math.round(valueStr);

	// Получаем первые две значащие цифры и количество нулей (множитель)
	// Для этого преобразуем число в строку без ведущих нулей
	const valueString = value.toString();

	// Первые две цифры
	let firstDigit = parseInt(valueString[0]);
	let secondDigit = parseInt(valueString[1] || '0');

	// Множитель — количество цифр после первых двух
	let power = valueString.length - 2;

	return colors[firstDigit] + ' ' + colors[secondDigit] + ' ' + colors[power] + ' gold';
}
