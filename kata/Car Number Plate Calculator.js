...
function findTheNumberPlate(customerID) {
	...
	const seriesIndex = Math.floor(customerID / 999);
	const number = (customerID % 999) + 1;

	const right = seriesIndex % 26;
	const middle = Math.floor(seriesIndex / 26) % 26;
	const left = Math.floor(seriesIndex / (26 * 26)) % 26;

	const letters = alphabet[right] + alphabet[middle] + alphabet[left];

	const numberStr = number.toString().padStart(3, '0');

	return letters + numberStr;
}