...
// Task
// Write a function lcs that accepts two strings and returns their longest common subsequence as a string.
// Performance matters.
// Examples
// lcs( "abcdef", "abc" ) => "abc"
// lcs( "abcdef", "acf" ) => "acf"
...
// Intermediate random tests have string arguments of 20 characters; hard random tests 40 characters;
// extreme 60 characters (characters are chosen out of 36 different ones).
// The reference algorithm solves all (12 fixed, 72 intermediate, 24 hard, 12 extreme) tests within ~150ms.
...
// When this occurs, return any of them.
...
function lcs(a, b) {
	...
	let prev = Array(n + 1).fill('');
	let curr = Array(n + 1).fill('');
	...
	for (let i = 0; i < m; i++) {
		for (let j = 0; j < n; j++) {
			if (a[i] === b[j]) {
				...
				curr[j + 1] = curr[j].length > prev[j + 1].length ? curr[j] : prev[j + 1];
			}
		}
		[prev, curr] = [curr, prev];
	}
